...
def sample_trajectory(memory, n_trajectories=128, use_attention=False):
...
def sample_and_pred(memory, model, batch_size, n_agents, n_trajectories=128, use_attention=False):
    sample_traj = random.sample(memory, n_trajectories)
    samples = Transition_e(*zip(*sample_traj))
    ...
    if not use_attention:
        ...
        x_train_tensor = torch.from_numpy(next_states).float().contiguous()
    _, y_time_hat, _ = model(x_train_tensor.to(device), hidden)
...
def make_train_step(model, loss_fn, optimizer, use_attention=False):
    ...
    def train_step(x, y, z):
        ...
        if not use_attention:
            ...
            yhat, y_time_hat, y_all = model(x, hidden)
        # Makes predictions
        else:
        ...
        if not use_attention:
            aux_loss = loss_fn(y[:,None], y_all)
            # loss_total = loss + 0.1*torch.norm(y_time_hat, p=1)
            loss_total = loss + 0.1*aux_loss
        else:
...
for i_episode in range(args.num_episodes):
            ...
            if total_numsteps % args.steps_per_actor_update == 0:
                for _ in range(args.updates_per_step):
                    ...
                    batch = sample_and_pred(memory_e.memory, reward_model, 
                                    ...
                                    n_trajectories=256, use_attention=use_attention)
    ...
    if (i_episode+1) % 1000 == 0 and (len(memory_e)>4000):
        ...
        for ii in range(1000):
            x_batch, y_batch, z_batch = sample_trajectory(memory_e.memory, 
                                                    n_trajectories=128, use_attention=use_attention)
